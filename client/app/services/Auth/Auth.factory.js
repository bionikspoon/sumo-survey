...
angular.module(MODULE_NAME, [LoopbackService]).factory('Auth', Auth);
...
function Auth($q, Admin, LoopBackAuth) {
  ...
  function login({ email, password, rememberMe }) {
    return Admin.login({ rememberMe }, { email, password }).$promise.finally(streamCurrentUser);
  }

  function logout() {
    return Admin.logout().$promise.finally(streamCurrentUser);
  }
  ...
  function getCurrentUser() {
    ...
    return getCurrentUser.data.finally(() => {
      getCurrentUser.data = null;
    }); // clear W.I.P.
  }
}
